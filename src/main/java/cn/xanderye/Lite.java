...
public class Lite {
    ...
    public static void main(String[] args) {
        // class列表文件
        String classesFilePath = "classes.txt";
        // rt文件夹
        String rtPath = "C:\\Program Files\\Java\\jre1.8.0_181\\lib\\rt";
        liteRt(classesFilePath, rtPath);
    }
    ...
    public static void liteRt(String classesFilePath, String rtPath) {
        ...
        // 检查目录
        rtPath = Util.checkPath(rtPath);
        newRtPath = Util.checkPath(newRtPath);
    ...
    public static void liteJfxRt(String classesFilePath, String rtPath) {
        ...
        try {
            ...
            System.out.println("已复制：" + count);
            Runtime runtime = Runtime.getRuntime();
            String[] jarPackage = {"jar", "cvf", "jfxrt.jar"};
            ...
            System.arraycopy(packages, 0, command, jarPackage.length, packages.length);
            runtime.exec(command);
            System.out.println("正在后台打包jfxrt.jar");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private static int copyClasses(String classesFilePath, String rtPath, String targetPath, boolean isJfx) {
        ...
        if (file.exists()) {
            try {
                ...
                if (count > 0) {
                    // 复制META-INF
                    Util.copyFolder(rtPath + "META-INF", targetPath + "META-INF");
                    if (isJfx) {
                        Util.copyFile(rtPath + "version.rc", targetPath + "version.rc");
                    }
                }
            } catch (Exception e) {
    ...
    private static String formatClassPath(String s, boolean isJfx) {
        ...
        if (s.contains("[Loaded ") && (s.contains(jar))) {
            s = Util.substringBetween(s, "[Loaded ", " ");
            if (s != null) {
